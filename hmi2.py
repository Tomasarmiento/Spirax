...
class HMISpirax:
    def __init__(self, root):
        self.root = root
        self.root.title("MOLTECH \u00b7 SPIRAX HMI")
    ...
    def _construir_ui(self):
        T.aplicar_estilos_ttk()
        T.crear_header(self.root, titulo='SPIRAX',
                       subtitulo='Celda de carga robotizada · KUKA KR16 + Fanuc 0i-TF')

        self.nb = ttk.Notebook(self.root)
        self.nb.pack(fill='both', expand=True, padx=10, pady=(8, 5))

        self.tab_operar = tk.Frame(self.nb, bg=T.BG)
        self.tab_calibrar = tk.Frame(self.nb, bg=T.BG)
        self.tab_torno = tk.Frame(self.nb, bg=T.BG)
        self.tab_log = tk.Frame(self.nb, bg=T.BG)
        ...
        # Acabado final del tema (botones planos, bordes, eyebrows)
        T.pulir(self.root)
    ...
    def _refrescar_boton_tipo(self, btn, tipo, seleccionado,
        ...
        if seleccionado:
            btn.configure(text=label, bg=T.AZUL, fg='white',
                          relief='flat', activebackground=T.AZUL)
        else:
            color_fg = T.OK if tiene_referencia(tipo, estacion) else T.ERR
            btn.configure(text=label, bg=T.BTN_BG, fg=color_fg,
                          relief='flat', activebackground=T.BTN_HOVER)

    # ------------------------------------------------------------------
    #  OPERAR
    # ------------------------------------------------------------------
    def _construir_tab_operar(self):
        cont = tk.Frame(self.tab_operar, bg=T.BG)
        cont.pack(fill='both', expand=True)

        col_izq = tk.Frame(cont, bg=T.BG)
        col_izq.pack(side='left', fill='both', expand=True,
                     padx=(15, 8), pady=10)

        col_der = tk.Frame(cont, bg=T.BG)
        ...
        f_tipo = tk.LabelFrame(col_izq, text=" TIPO DE PIEZA (Receta) ",
                               font=T.F_BOLD_LG,
                               bg=T.BG, fg='white', padx=10, pady=10)
        f_tipo.pack(fill='x', pady=5)

        c_tipos = tk.Frame(f_tipo, bg=T.BG)
        c_tipos.pack()
        for i in range(1, 7):
        ...
        f_cinta = tk.LabelFrame(col_izq, text=" CINTA (Estacion 1) ",
                                 font=T.F_BOLD_LG,
                                 bg=T.BG, fg=T.INFO_TXT,
                                 padx=10, pady=10)
        f_cinta.pack(fill='x', pady=8)

        c_modo_c = tk.Frame(f_cinta, bg=T.BG)
        ...
        tk.Label(f_cinta,
                 ...
                 font=T.F_XS_IT,
                 bg=T.BG, fg=T.FG_MUTED).pack(anchor='w', pady=(8, 2))

        # Checkbox de activacion + orientaciones manuales
        c_orient_row = tk.Frame(f_cinta, bg=T.BG)
        ...
        self.chk_cinta_manual = tk.Checkbutton(
            ...
            font=T.F_SM,
            bg=T.BG, fg='white',
            selectcolor=T.BG_DEEP,
            activebackground=T.BG,
            activeforeground='white')
        self.chk_cinta_manual.grid(row=0, column=0, padx=(0, 10))

        c_orient = tk.Frame(c_orient_row, bg=T.BG)
        ...
        c_modo_m = tk.Frame(f_mesa, bg=T.BG)
        ...
        tk.Label(c_man, text="Fila:", bg=T.BG, fg='white',
                 font=T.F_SM).grid(row=0, column=1, padx=5)
        ...
        tk.Label(c_man, text="Columna:", bg=T.BG, fg='white',
                 font=T.F_SM).grid(row=0, column=3, padx=15)
        ...
        c_vista = tk.Frame(f_img_mesa, bg=T.BG)
        c_vista.pack(fill='x', padx=5, pady=(0, 4))

        self.var_vista_mesa = tk.StringVar(value="matriz")

        tk.Label(c_vista, text="Vista:", font=T.F_XS,
                 bg=T.BG, fg=T.FG_SOFT).pack(side='left', padx=(0, 8))
        ...
        frame_canvas_m = tk.Frame(f_img_mesa, bg=T.BG_DEEP,
        ...
        self.canvas_matriz = tk.Canvas(frame_canvas_m,
                                        bg=T.BG_DEEP, highlightthickness=0)
        # Vista 2: Label con imagen de bg subtraction
        self.canvas_bg_mesa = tk.Label(frame_canvas_m, bg=T.BG_DEEP,
                                        fg=T.FG_MUTED,
        ...
        for fi in range(GRILLA_FILAS):
            for ci_canvas in range(GRILLA_COLS):
                ...
                if ocupada and elegida:
                    fill = T.CELDA_ELEGIDA
                    outline = T.CELDA_ELEGIDA
                elif ocupada:
                    fill = T.CELDA_OCUPADA
                    outline = T.CELDA_OCUPADA_BORDE
                else:
                    fill = T.CELDA_VACIA
                    outline = T.CELDA_VACIA_BORDE
    ...
    def _construir_tab_calibrar(self):
        cont = tk.Frame(self.tab_calibrar, bg=T.BG)
        cont.pack(fill='both', expand=True, padx=15, pady=10)

        # Banner
        f_banner = tk.Frame(cont, bg=T.BG_DEEP, height=60)
        ...
        c_tog = tk.Frame(f_toggle, bg=T.BG)
        c_tog.pack()
        ...
        f_sel = tk.LabelFrame(cont, text=" TIPO A CALIBRAR ",
                              font=T.F_BOLD,
                              bg=T.BG, fg='white', padx=10, pady=8)
        f_sel.pack(fill='x', pady=(0, 10))

        c_tipos_cal = tk.Frame(f_sel, bg=T.BG)
        c_tipos_cal.pack()
        for i in range(1, 7):
        ...
        col_der_outer = tk.Frame(cuerpo, bg=T.BG)
        col_der_outer.grid(row=0, column=1, sticky='ns')

        col_der_canvas = tk.Canvas(col_der_outer, bg=T.BG,
        ...
        col_der = tk.Frame(col_der_canvas, bg=T.BG)
        ...
        # Preview + referencia
        f_visual = tk.Frame(col_izq, bg=T.BG)
        ...
        f_prev = tk.LabelFrame(f_visual, text=" PREVIEW EN VIVO ",
                               font=T.F_BOLD,
                               bg=T.BG, fg='white', padx=5, pady=5)
        f_prev.grid(row=0, column=0, sticky='nsew', padx=(0, 5))

        frame_canvas = tk.Frame(f_prev, bg=T.BG_DEEP,
                                width=PANEL_W, height=PANEL_H)
        ...
        c_prev_btns = tk.Frame(f_prev, bg=T.BG)
        ...
        frame_canvas_ref = tk.Frame(f_ref, bg=T.BG_DEEP,
                                    width=PANEL_W, height=PANEL_H)
        ...
        tk.Label(f_sld, text="Exposicion:", font=T.F_SM,
                 bg=T.BG, fg='white').pack(anchor='w')
        ...
        tk.Label(f_sld, text="Ganancia:", font=T.F_SM,
                 bg=T.BG, fg='white').pack(anchor='w', pady=(5, 0))
        ...
        tk.Label(f_sld, text="Threshold BG Subtract:", font=T.F_SM,
                 bg=T.BG, fg='white').pack(anchor='w', pady=(5, 0))
    ...
    def _construir_tab_torno(self):
        ...
        tk.Label(f_cola,
                 ...
                 font=T.F_XS_IT,
                 bg=T.BG, fg=T.FG_MUTED).pack(anchor='w', pady=(0, 4))

        # Listbox
        f_lb = tk.Frame(f_cola, bg=T.BG)
        f_lb.pack(fill='both', expand=True)

        sb = tk.Scrollbar(f_lb)
        sb.pack(side='right', fill='y')

        self.lst_cola = tk.Listbox(f_lb, bg=T.BG_DEEP, fg=T.FG_SOFT,
                                    font=("Consolas", 10),
        ...
        # Controles cola
        c_ctrl = tk.Frame(f_cola, bg=T.BG)
        ...
        tk.Label(f_conn,
                 ...
                 font=("Consolas", 10),
                 bg=T.BG, fg=T.FG_SOFT).pack(anchor='w')
        ...
        tk.Label(f_conn,
                 ...
                 font=("Consolas", 9),
                 bg=T.BG, fg=T.FG_MUTED).pack(anchor='w', pady=(2, 8))

        c_btns_conn = tk.Frame(f_conn, bg=T.BG)
        ...
        self.txt_torno_status = tk.Text(
            f_status, height=10, bg=T.BG_DEEP, fg=T.FG_SOFT,
        ...
        c_insp = tk.Frame(f_insp, bg=T.BG)
        c_insp.pack(fill='x')

        tk.Label(c_insp, text="Macro #:", font=T.F_SM,
                 bg=T.BG, fg='white').grid(row=0, column=0, padx=2)
        ...
        tk.Button(c_insp, text="Leer",
                  ...
                  font=T.F_XS).grid(row=0, column=2, padx=2)

        tk.Label(c_insp, text="Valor:", font=T.F_SM,
                 bg=T.BG, fg='white').grid(row=0, column=3, padx=2)
        ...
        tk.Button(c_insp, text="Escribir",
                  ...
                  font=T.F_XS,
                  bg=T.ABAJO, fg='white').grid(row=0, column=5, padx=2)
    ...
    def _refrescar_cola_torno(self):
        ...
        for i, item in enumerate(items):
            ...
            if i == 0:
                self.lst_cola.itemconfig(0, fg=T.WARN_TXT)
    ...
    def _construir_tab_log(self):
        cont = tk.Frame(self.tab_log, bg=T.BG)
        cont.pack(fill='both', expand=True, padx=15, pady=10)

        self.txt_log = tk.Text(cont, bg=T.BG_DEEP, fg=T.FG_SOFT,
                               font=("Consolas", 10), wrap='word')
        self.txt_log.pack(fill='both', expand=True)

        c_btns = tk.Frame(cont, bg=T.BG)
        ...
        if est["modo_auto_cinta"]:
            ...
            for btn in self.btns_orient:
                btn.configure(state='disabled',
                              bg=T.PANEL, fg=T.NEUTRO,
                              relief='flat',
                              disabledforeground=T.NEUTRO)
    ...
    def _refrescar_botones_orientacion(self):
        ...
        colores = {"ARRIBA": T.OK, "ABAJO": T.ABAJO,
                   "VACIO": T.NEUTRO}
        for btn in self.btns_orient:
            txt = btn['text']
            if txt == activa:
    ...
    def _cambiar_modo_cinta(self, auto):
        if auto:
            ...
            self.btn_auto_cinta.configure(bg=T.OK, fg='white',
                                           relief='flat')
            self.btn_manual_cinta.configure(bg=T.BTN_BG, fg=T.FG,
                                             relief='flat')
        else:
            estado.set_modo_auto_cinta(False)
            self.btn_manual_cinta.configure(bg=T.AZUL, fg='white',
                                             relief='flat')
            self.btn_auto_cinta.configure(bg=T.BTN_BG, fg=T.FG,
                                           relief='flat')
    ...
    def _cambiar_modo_mesa(self, auto):
        if auto:
            ...
            self.btn_auto_mesa.configure(bg=T.OK, fg='white',
                                          relief='flat')
            self.btn_manual_mesa.configure(bg=T.BTN_BG, fg=T.FG,
                                            relief='flat')
        else:
            estado.set_modo_auto_mesa(False)
            self.btn_manual_mesa.configure(bg=T.AZUL, fg='white',
                                            relief='flat')
            self.btn_auto_mesa.configure(bg=T.BTN_BG, fg=T.FG,
                                          relief='flat')
    ...
    def _set_estacion_calibrar(self, estacion):
        ...
        if estacion == "cinta":
            self.btn_estac_cinta.configure(bg=T.AZUL, fg='white',
                                            relief='flat')
            self.btn_estac_mesa.configure(bg=T.BTN_BG, fg=T.FG,
                                           relief='flat')
        else:
            self.btn_estac_mesa.configure(bg=T.WARN, fg=T.BG_DEEP,
                                           relief='flat')
            self.btn_estac_cinta.configure(bg=T.BTN_BG, fg=T.FG,
                                            relief='flat')
    ...
    def _seleccionar_tipo_calibrar(self, tipo):
        ...
        if tiene_referencia(tipo, est):
            self.lbl_banner_calibrar.configure(
                ...
                fg=T.INFO_TXT if est == "cinta" else T.WARN)
            ...
            self.lbl_estado_calibrar.configure(
                text="(falta capturar referencia)", fg=T.ERR_TXT)
        ...
        if est == "mesa":
            self.scl_umbral.configure(state='normal',
                                       fg='white', troughcolor=T.BG_DEEP)
            ...
            self.scl_umbral.configure(state='disabled',
                                       fg=T.FG_DIM, troughcolor=T.PANEL_DIM)
            self.lbl_umbral_titulo.configure(fg=T.FG_DIM)
            self.scl_margen.configure(state='disabled',
                                       fg=T.FG_DIM, troughcolor=T.PANEL_DIM)
            self.lbl_margen_titulo.configure(fg=T.FG_DIM)
    ...
    def _iniciar_preview(self):
        ...
        self.btn_preview_start.configure(text="DETENER PREVIEW", bg=T.ERR)
    ...
    def _detener_preview(self):
        self._preview_activo = False
        self.btn_preview_start.configure(text="INICIAR PREVIEW", bg=T.AZUL)
    ...
    def _refrescar_estado_archivos(self):
        ...
        if os.path.exists(ref_path):
            ...
            self.lbl_ref.configure(text="referencia FALTA", fg=T.ERR_TXT)
        if os.path.exists(cfg_path):
            self.lbl_cfg.configure(text="config OK", fg=T.OK_TXT)
        else:
    ...
    def _actualizar_estado_loop(self):
        ...
        if estado.robot_conectado:
            self.lbl_robot.configure(text="Robot: CONECTADO", fg=T.OK_TXT)
        else:
            self.lbl_robot.configure(text="Robot: desconectado", fg=T.ERR_TXT)

        self.lbl_consultas.configure(text=f"Consultas recibidas: {estado.consultas}")

        if detector.esta_activo():
            self.lbl_camara.configure(text="Camara: ACTIVA", fg=T.OK_TXT)
        else:
            self.lbl_camara.configure(text="Camara: apagada", fg=T.ERR_TXT)
        ...
        if orient is not None:
            colores = {"ARRIBA": T.OK_TXT, "ABAJO": T.WARN_TXT2,
                       "VACIO": T.FG_SOFT, "DUDOSO": T.AMARILLO,
                       "N/A": T.ERR_TXT}
        ...
        if fila_m > 0 and col_m > 0:
            self.lbl_deteccion_mesa.configure(
                text=f"Elegida: ({fila_m}, {col_m})", fg=T.WARN_TXT)
        else:
        ...
        try:
            if torno.conectado:
                ...
                self.lbl_torno_estado.configure(
                    text=f"Estado: {err[:40]}", fg=T.ERR_TXT)
    ...
    def _abrir_ventana_zoom(self, panel_bgr, titulo):
        ...
        toolbar = tk.Frame(win, bg=T.BG)
        toolbar.pack(side='top', fill='x')

        lbl_zoom = tk.Label(toolbar, text="Zoom: 100%",
                             font=T.F_SM, bg=T.BG, fg='white',
                             padx=10)
        lbl_zoom.pack(side='left')

        tk.Label(toolbar, text="(rueda del mouse: zoom | arrastrar: pan)",
                 font=T.F_XS_IT,
                 bg=T.BG, fg=T.FG_MUTED).pack(side='left', padx=10)

        # Canvas con scrollbars
        c_canvas = tk.Frame(win, bg=T.BG_DEEP)
        ...
        canvas = tk.Canvas(c_canvas, bg=T.BG_DEEP,
        ...
        tk.Button(toolbar, text="−", width=3, command=zoom_out,
                  font=T.F_BOLD_LG).pack(side='right', padx=2)
