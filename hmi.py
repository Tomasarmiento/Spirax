...
class EstadoCompartido:
    ...
    def __init__(self):
        ...
        # Pallet EN PROCESO (el que esta en el spot 2 / robot):

        # al llegar una foto nueva del spot 1, la anterior pasa aca.

        self.panel_en_proceso = None

        self.orient_en_proceso = None
    ...
    def set_deteccion_cinta(self, orientacion, ratio, panel, error=None):
        ...
        with self.lock:
            ...
            # La foto ANTERIOR corresponde al pallet que ya avanzo hacia el

            # spot 2 (robot): pasa al panel "en proceso" antes de pisarla.

            if self.ultimo_panel_cinta is not None:

                self.panel_en_proceso = self.ultimo_panel_cinta

                self.orient_en_proceso = self.ultima_deteccion_cinta
...
# Operacion pendiente de encolar al torno: (tipo, op, rosca) o None.

# Se guarda al responder la foto de la cinta y se ENCOLA recien cuando el

# SPS avisa que el robot termino la rutina (Recuperar=2). Asi la cola solo

# contiene operaciones efectivamente realizadas.
...
def _extraer_recuperar(mensaje):

    """Devuelve 1 si el request trae <Recuperar>1</Recuperar>, si no 0.

    Lo manda el SPS al retomar automatico cuando quedo un pallet en el

    spot 2: hay que cambiar el op del ultimo item de la cola a 3.

    """

    import re

    m = re.search(r"<Recuperar>\s*(\d+)\s*</Recuperar>", mensaje)
...
def manejar_robot(conn, addr, log_callback):
    ...
    try:
        ...
        while not estado.shutdown:
            ...
            recuperar = _extraer_recuperar(mensaje)



            # --- AVISOS DEL SPS (campo Recuperar) ---

            # 1 = gap/recuperacion: pallet en spot 2 en estado desconocido.

            #     Encolar op=3 (el torno lo deja pasar, se recicla) y

            #     descartar el pendiente (ese pallet se re-evaluara).

            # 2 = robot termino la rutina: encolar la operacion pendiente
            ...
            if recuperar in (1, 2):
                ...
                if recuperar == 1:
                    ...
                    with estado.lock:
                        ...
                        estado.panel_en_proceso = None

                        estado.orient_en_proceso = None

                    torno.encolar(est["tipo"], 3, rosca)

                    log_callback("[RECUPERACION] Encolado op=3 (pallet en "

                                 "gap, el torno lo deja pasar)")
                ...
                respuesta = ("<Response><Tipo>0</Tipo>"

                             "<Orientacion>VACIO</Orientacion>"

                             "<Fila>0</Fila><Columna>0</Columna></Response>")

                try:

                    conn.sendall(respuesta.encode('utf-8'))

                    log_callback(f"--> Enviado (aviso):  {respuesta}")
            ...
            if num_estacion == 1:
                ...
                # NO encolar aca: guardar como pendiente. Se encola recien

                # cuando el robot TERMINO la rutina (aviso Recuperar=2).
...
def consultar_vision_cinta(tipo, log_callback):
    ...
    """Devuelve ARRIBA/ABAJO/VACIO. Abre la camara, captura y la cierra

    (analizar_atomico) para no competir con la otra camara por el cable."""
...
def consultar_vision_mesa(tipo, log_callback):
    ...
    """Devuelve (fila, columna). Abre la camara, captura y la cierra

    (analizar_atomico) para no competir con la otra camara por el cable."""

    detector = detector_mesa
...
class HMISpirax:
    ...
    def _construir_tab_operar(self):
        ...
        # Toggle Actual (spot 1) / Anterior (en proceso, spot 2)
        c_vista_cinta = tk.Frame(f_img_cinta, bg='#2b2b2b')
        c_vista_cinta.pack(fill='x', padx=5, pady=(0, 4))

        self.var_vista_cinta = tk.StringVar(value="actual")

        tk.Label(c_vista_cinta, text="Vista:", font=("Arial", 9),
                 bg='#2b2b2b', fg='#cfcfcf').pack(side='left', padx=(0, 8))

        tk.Radiobutton(
            c_vista_cinta, text="Actual (spot 1)",
            variable=self.var_vista_cinta, value="actual",
            font=("Arial", 9), bg='#2b2b2b', fg='white',
            selectcolor='#1a1a1a', activebackground='#2b2b2b',
            activeforeground='white').pack(side='left')

        tk.Radiobutton(
            c_vista_cinta, text="Anterior (en proceso)",
            variable=self.var_vista_cinta, value="anterior",
            font=("Arial", 9), bg='#2b2b2b', fg='white',
            selectcolor='#1a1a1a', activebackground='#2b2b2b',
    ...
    def _construir_tab_torno(self):
        ...
        tk.Label(f_conn,
                 ...
                 text=f"Macros: #{torno.config['macro_tipo']}=pieza  "
                      ...
                      f"#{torno.config['macro_cola']}=cola",
    ...
    def _cambiar_modo_cinta(self, auto):
        ...
        if auto:
            ...
            # NO abrir la camara aca. En automatico la camara se abre sola

            # en cada consulta (analizar_atomico), captura y cierra. Abrirla

            # aca dejaria el lock global tomado y colgaria la otra estacion.
    ...
    def _cambiar_modo_mesa(self, auto):
        ...
        if auto:
            ...
            # NO abrir la camara aca (ver comentario en _cambiar_modo_cinta).
    ...
    def _ajustar_recorte(self):
        ...
        # una orientacion (rotacion/flip) distinta a la activa ahora â en
    ...
    def _actualizar_estado_loop(self):
        ...
        with estado.lock:
            panel_p = estado.panel_en_proceso
            orient_p = estado.orient_en_proceso

        colores = {"ARRIBA": "#7fff7f", "ABAJO": "#ffa500",
                   "VACIO": "#cccccc", "DUDOSO": "#ffff7f",
                   "N/A": "#ff6b6b"}

        if self.var_vista_cinta.get() == "anterior":
            # Vista ANTERIOR: la foto del pallet que trabaja el robot
            if panel_p is not None:
                self._mostrar_panel_cinta(panel_p)
                self.lbl_deteccion_cinta.configure(
                    text=f"Resultado: {orient_p} (en proceso)" if orient_p
                         else "Resultado: --- (en proceso)",
                    fg=colores.get(orient_p, 'white'))
            else:
                self.canvas_cinta.configure(image='',
                                             text="(sin pallet en proceso)",
                                             fg='#888888')
                self._img_cinta = None
                self.lbl_deteccion_cinta.configure(
                    text="Resultado: ---", fg='white')
            self.lbl_ratio_cinta.configure(text="Ratio: ---")
        else:
            # Vista ACTUAL: la ultima foto del spot 1
    ...
    def _abrir_ventana_zoom(self, panel_bgr, titulo):
        ...
        tk.Button(toolbar, text="â", width=3, command=zoom_out,
